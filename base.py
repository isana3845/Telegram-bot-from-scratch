...
class Bot:
            ...
            if photo:
                ...
                if file_path:
                    ...
                    async with self.session.get(
                        f"https://api.telegram.org/file/bot{self.bot_token}/{file_path['result']['file_path']}"
                    ) as aii:
    ...
    async def _requests(self, method, json_data):
        if not self.session:
            raise Exception("Сессия не запущена. Вызови await bot.start_session()")
    ...
    async def send_photo(self, chat_id, photo_path):
        if not self.session:
            raise Exception("Сессия не запущена.")
